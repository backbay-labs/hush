 ...
 * `fixtures/log/`.
 *
 * The vectors were generated by the Rust reference under the fixed inputs of
 * `fixtures/receipts/expected/README.md`. This SDK must verify every
 * `valid/` file, reject every `invalid/` one at the line its name ends with,
 * and -- because the entry hash is over the canonical form -- reproduce the
 * same chain byte for byte when it writes one itself.
 */
