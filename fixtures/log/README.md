...
- `invalid/*.jsonl` MUST be rejected, and a verifier MUST identify the line the file name ends with as
  ...
  `malformed-receipt-line-2`, `bad-signature-line-4`). `malformed-receipt-line-2` is hash-consistent
  throughout, so only the receipt payload itself -- which carries nothing but `receipt_version` --
  fails it (log spec section 8, step 8). The signature vector needs the keyring; the others break
  without one.
