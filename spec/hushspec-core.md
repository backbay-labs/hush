...
Scalar resolution MUST follow the complete Core schema, including numbers and nulls. Leading-zero decimal `010` denotes ten; `0o12` and `0xA` also denote ten. Sexagesimal `1:20`, binary `0b1010`, underscore-bearing `1_0`, signed radix forms such as `+0xA`, and timestamp-looking text are strings. Quoting preserves string typing. A string MUST NOT be coerced into a numeric or boolean property.

Explicit tags MUST resolve to Core types (`str`, `null`, `bool`, `int`, `float`, `seq`, or `map`) and their payloads MUST have the corresponding Core representation. Invalid tag payloads MUST be refused, including inside free-form context; parsers MUST NOT fall back to strings or legacy YAML 1.1 constructors. `!!float 010` denotes decimal ten, while `!!float 0x10` is invalid. Non-Core tags, including `!!timestamp` and application tags, MUST be refused. The non-specific scalar tag `!` denotes a string, so `! 010` preserves the string `010`.

For a schema property of type `integer`, a finite number with no fractional part is accepted regardless of its YAML tag: `10`, `10.0`, and `1e1` denote the same integer. Its magnitude MUST NOT exceed 2^53 - 1 (9007199254740991), including when float syntax is used; individual properties may impose tighter bounds. This portable integer-field bound does not apply to `number` properties or free-form condition context, which retain finite binary64 values. Integer-syntax literals remain subject to the canonical specification's safe-integer bound everywhere. NaN and infinities, including exponent overflow, MUST be refused before policy construction.

Raw source vectors: `fixtures/core/raw-yaml/scalars.json`. The JSON container carries complete YAML source strings; runners pass those strings directly to each SDK parser without decoding and re-emitting YAML.
...
1. **TLS only.** The scheme MUST be `https`. An `http://` reference MUST be refused outright, never upgraded. Certificate verification MUST be on by default; an option to disable it exists for test harnesses only and MUST be documented as unsafe. An engine MAY offer a second test-only option that admits a loopback address, and plain `http` to a loopback address, so a local test server can be reached; that option MUST be off by default, MUST reach loopback addresses and nothing else, and MUST be documented as unsafe.
...
8. **Bounded time.** Establishing the connection and reading the response MUST have separate budgets, so a server that accepts a connection and then stalls does not inherit the connect timeout's patience. DNS resolution and opening the TCP connection MUST share the connection budget; completing DNS MUST NOT start a fresh connection budget. A DNS timeout MUST prevent any later answer from initiating a request. Implementations using non-cancellable system resolvers MUST bound outstanding resolver work and fail closed when that capacity is exhausted. The reference default is 10 seconds for each budget.
...
**Runtime context.** The engine supplies an object with the following top-level keys, each OPTIONAL: `user` (object), `environment` (string), `deployment` (object), `agent` (object), `session` (object), `request` (object), `custom` (object), `counters` (object of string to non-negative integer, consulted by `rate` conditions), and `current_time` (used only for deterministic testing). `current_time` is an RFC 3339 timestamp in the portable grammar `YYYY-MM-DDTHH:MM:SS[.fraction](Z / +HH:MM / -HH:MM)`, with ASCII digits, uppercase `T` and `Z`, a calendar year from `0001` through `9999`, and an offset hour/minute from `00` through `23`/`59`. The resulting UTC instant and its conversion into the window's configured `timezone` MUST both remain in year `0001` through `9999`. For compatibility, the reference SDKs also accept the same complete date-time with no suffix and interpret it as UTC. Date-only values, a space separator, missing seconds, lowercase markers, leap seconds and normalized calendar or offset fields are not accepted. A `context` condition key such as `user.role` resolves `user` then `role`; the key `environment` resolves the top-level string.
