...
Requirements (Core Section 2.6.4): HTTPS only, with certificate verification on and `http://` refused outright; an optional exact, case-insensitive host allowlist checked before DNS; every address the host resolves to checked against the blocked networks Core Section 2.6.4 tabulates, with IPv4-mapped and IPv4-compatible IPv6 forms unwrapped and judged on the address inside and an unparseable address treated as blocked; the checked address pinned for the connection while the host name still carries SNI, certificate validation and the `Host` header; no redirects; separate connect and read budgets; a capped body; `ETag` revalidation that never serves a cached document except on a `304`; and `<url>.sig` fetched under those same rules. Digest pins (Core Section 2.3) and detached signatures (Signing Section 6.5) supply integrity and authenticity, which no transport rule can. Engines without a loader MUST refuse URL references rather than treat them as paths.

Residual risk: the blocked-network list is a list of addresses, not of services. A policy server on a routable address that an operator did not intend to expose is reachable, and an internal service on public address space is not covered. Pinning closes the rebinding window between the check and the connection, but a name whose *first* answer is already hostile is refused only because the address check refuses it. Operators SHOULD narrow the allowlist to the hosts they actually publish from, SHOULD prefer digest pins or signatures over trust in the transport, and SHOULD load remote policies at deployment time rather than per request.
