...
| **Identical reason codes.** The 11 signing reasons, the 5 bundle reasons and the resolve reasons are the same strings everywhere. | `fixtures/signing/vectors.yaml` (18 cases) and `fixtures/bundle/vectors.yaml` (8 cases), each asserting the exact code, run by all four. |
