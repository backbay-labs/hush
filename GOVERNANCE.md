...
`0`. It does not gate merges in this repository. Making it enforceable is tracked as RFC
09 package P2-11.

## How Specification Changes Happen

HushSpec follows a **spec-first, fixture-first** process, per RFC 09
(`docs/plans/09-compliance-as-code-plan.md`) Section 2, Operating Rule 1:
...
1. **Propose.** Non-trivial changes to the specification (`spec/`), schemas
   ...
   `docs/plans/`. Existing RFCs (`01`-`09`) are the model: state the problem, the gap
   in current behavior, the proposed normative change, and the work packages needed to
...
4. **Implement across SDKs.** Rust lands first (it is the oracle for the conformance
   testkit and differential fuzzer), then TypeScript, Python, and Go. A behavior change
   is not considered complete until all four SDKs pass the same fixture and, where
   applicable, `hushspec-testkit`'s differential fuzzer (`hushspec-difftest`) shows no
   divergence.
...
security-considerations section and stable registries, is tracked as RFC 09 package
P5-03). Until then, treat a spec/implementation divergence or an ambiguous normative
sentence as a bug: open an issue or a pull request against `spec/` describing the
divergence and the proposed clarification. Errata-level fixes (patch versions) must not
change document validity or evaluation semantics for previously-valid documents; anything
that would is a breaking (minor-version) change and needs an RFC.
...
expected to apply that discipline manually until P2-11 lands. CI must be green
(`.github/workflows/ci.yml`: generated-sources, per-language unit tests, shared fixtures
across all four SDKs, smoke snippets, cross-SDK roundtrip, differential fuzz, bench
thresholds, docs build) before a pull request is merged.
